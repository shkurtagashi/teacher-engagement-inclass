...
import com.example.android.teacher.data.Surveys.PostLectureSurveyContract;
...
//        onCreate(db);
    ...
    public void deleteUserRegistration(User user){
        SQLiteDatabase db = this.getWritableDatabase();
    ...
    public void addEmpaticaE4Data(EmpaticaE4 e4) {
        ...
        try {
            ContentValues values = new ContentValues();
            values.put(E4DataEntry._USERNAME, e4.getUsername());
            ...
            db.setTransactionSuccessful();
            System.out.println("USER DATA INSERTED: "+ values);
            Log.v("WHATEVERRRR", "New Empatica E4 device with name: " + e4.getName() +
                    " and api key: " + e4.getApiKey() + " for user with username: " + e4.getUsername());
        } catch (SQLException e) {
            e.printStackTrace();
